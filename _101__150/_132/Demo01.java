...
public class Demo01 {
    ...
    private int minCut(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            partition(s, i, i, 0, s.length() - 1, dp);
            if (i + 1 < s.length() && s.charAt(i) == s.charAt(i + 1)) partition(s, i, i + 1, 0, s.length() - 1, dp);
        }
    ...
    private int[] partition(String s, int l, int r, int left, int right, int[][] dp) {
